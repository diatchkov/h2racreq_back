CLASS LHC_ZI_H2R_ACC_BUILD_S DEFINITION FINAL INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE AccBuildAll\_AccBuild
          ENTITIES_UPDATE FOR UPDATE AccBuild.
ENDCLASS.

CLASS LHC_ZI_H2R_ACC_BUILD_S IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE ZI_H2R_ACC_BUILD\_AccBuildText,
          text_for_existing_entity TYPE TABLE FOR CREATE ZI_H2R_ACC_BUILD\_AccBuildText,
          text_update              TYPE TABLE FOR UPDATE ZI_H2R_ACC_BUILDT.
    ...
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK ZI_H2R_ACC_BUILD\_AccBuildText,
    ...
    LOOP AT entity-%TARGET ASSIGNING FIELD-SYMBOL(<target>).
      ...
      INSERT VALUE #( %CID_REF = <target>-%CID
                      ...
                      %TARGET = VALUE #( (
                        ...
                        %CONTROL-BuildName = <target>-%CONTROL-BuildName ) ) )
                   INTO TABLE text_for_new_entity.
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF ZI_H2R_ACC_BUILD_S
      ENTITY AccBuild
        CREATE BY \_AccBuildText
        FROM text_for_new_entity
        RELATING TO entities_create BY relates_create.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF ZI_H2R_ACC_BUILD_S
        ENTITY AccBuild BY \_AccBuildText
          FROM CORRESPONDING #( entities_update )
          LINK DATA(link).
      LOOP AT entities_update INTO DATA(update) WHERE %CONTROL-BuildName = if_abap_behv=>mk-on.
        DATA(tabix) = sy-tabix.
        ...
        IF line_exists( link[ KEY draft source-%TKY  = CORRESPONDING #( update-%TKY )
          ...
          APPEND VALUE #( %TKY = text_tky
                          ...
                          %CONTROL = VALUE #( BuildName = update-%CONTROL-BuildName )
          ) TO text_update.
        ...
        ELSE.
          ...
          APPEND VALUE #( %TKY = CORRESPONDING #( update-%TKY )
                          ...
                          %TARGET  = VALUE #( (
                            ...
                            BuildName = update-BuildName
                            %CONTROL-Spras = if_abap_behv=>mk-on
                            %CONTROL-BuildName = update-%CONTROL-BuildName
                          ) )
