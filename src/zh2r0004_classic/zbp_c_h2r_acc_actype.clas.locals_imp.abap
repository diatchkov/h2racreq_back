CLASS LHC_ZI_H2R_ACC_ACTYPE_S DEFINITION FINAL INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE AccTypeAll\_AccType
          ENTITIES_UPDATE FOR UPDATE AccType.
ENDCLASS.

CLASS LHC_ZI_H2R_ACC_ACTYPE_S IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE ZI_H2R_ACC_ACTYPE\_AccTypeText,
          text_for_existing_entity TYPE TABLE FOR CREATE ZI_H2R_ACC_ACTYPE\_AccTypeText,
          text_update              TYPE TABLE FOR UPDATE ZI_H2R_ACC_ACTYPT.
    ...
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK ZI_H2R_ACC_ACTYPE\_AccTypeText,
    ...
    LOOP AT entity-%TARGET ASSIGNING FIELD-SYMBOL(<target>).
      ...
      INSERT VALUE #( %CID_REF = <target>-%CID
                      %IS_DRAFT = <target>-%IS_DRAFT
                        %KEY-Actype = <target>-%KEY-Actype
                      %TARGET = VALUE #( (
                        %CID = |CREATETEXTCID{ sy-tabix }|
                        %IS_DRAFT = <target>-%IS_DRAFT
                        Spras = sy-langu
                        ActypeText = <target>-ActypeText
                        %CONTROL-Spras = if_abap_behv=>mk-on
                        %CONTROL-ActypeText = <target>-%CONTROL-ActypeText ) ) )
                   INTO TABLE text_for_new_entity.
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF ZI_H2R_ACC_ACTYPE_S
      ENTITY AccType
        CREATE BY \_AccTypeText
        FROM text_for_new_entity
        RELATING TO entities_create BY relates_create.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF ZI_H2R_ACC_ACTYPE_S
        ENTITY AccType BY \_AccTypeText
          FROM CORRESPONDING #( entities_update )
          LINK DATA(link).
      LOOP AT entities_update INTO DATA(update) WHERE %CONTROL-ActypeText = if_abap_behv=>mk-on.
        DATA(tabix) = sy-tabix.
        ...
        IF line_exists( link[ KEY draft source-%TKY  = CORRESPONDING #( update-%TKY )
          ...
          APPEND VALUE #( %TKY = text_tky
                          ...
                          ActypeText = update-ActypeText
                          %CONTROL = VALUE #( ActypeText = update-%CONTROL-ActypeText )
          ) TO text_update.
        ...
        ELSE.
          ...
          APPEND VALUE #( %TKY = CORRESPONDING #( update-%TKY )
                          ...
                          %TARGET  = VALUE #( (
                            ...
                            ActypeText = update-ActypeText
                            %CONTROL-Spras = if_abap_behv=>mk-on
                            %CONTROL-ActypeText = update-%CONTROL-ActypeText
                          ) )
