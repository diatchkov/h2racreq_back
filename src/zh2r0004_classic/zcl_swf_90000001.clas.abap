CLASS ZCL_SWF_90000001 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS read_meta
      FOR TABLE FUNCTION ZSWFM_90000001.
  PROTECTED SECTION.
  PRIVATE SECTION.
ENDCLASS.


CLASS ZCL_SWF_90000001 IMPLEMENTATION.
...
* Containter Element 'ACCREQUEST' refers to CDS View 'ZR_H2R_ACC_ACREQ'
    DECLARE lv___KEY1 "$ABAP.type( SYSUUID_X16 )";  /* Field ACCREQUESTID */
...
* Each simple data container attribute gets selected now
    ...
    BEGIN
      DECLARE EXIT HANDLER FOR SQLEXCEPTION SELECT ::SQL_ERROR_CODE,  ::SQL_ERROR_MESSAGE FROM PUBLIC.DUMMY;
      SELECT value INTO lv___KEY FROM :lt_simple_attr WHERE name =  'ACCREQUEST';
      SELECT SUBSTRING( lv___KEY,1,32) INTO lv___KEY1 FROM PUBLIC.DUMMY;
    END;
